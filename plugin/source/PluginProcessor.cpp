...
constexpr auto kPresetPathProperty = "presetPath";
...
constexpr auto kDefaultPresetDirectoryName = "Adaptive Echo Presets";

juce::File normalizePresetSaveFile(const juce::String &path) {
  auto file = juce::File(path);
  if (!file.hasFileExtension(
          adaptive_echo::plugin_files::kPresetFileExtension)) {
...
juce::File resolvePresetLoadFile(const juce::String &path) {
  auto file = juce::File(path);
  ...
  return file.withFileExtension(
      adaptive_echo::plugin_files::kPresetFileExtension);
}

float getNormalizedParameterValue(
...
bool getBoolParameterValue(const juce::AudioProcessorValueTreeState &parameters,
                           const juce::StringRef parameterId,
                           bool fallbackValue) {
  if (auto *parameter = parameters.getRawParameterValue(parameterId)) {
...
adaptive_echo::FilterParameters<float>
makeFilterParameters(juce::AudioProcessorValueTreeState &parameters,
  ...
  normalized[0] = getNormalizedParameterValue(parameters, highPassCutoffId,
                                              kDefaultBypassHighPassCutoff);
  normalized[1] = getNormalizedParameterValue(parameters, highPassSlopeId,
                                              kDefaultBypassHighPassSlope);
  normalized[2] = getNormalizedParameterValue(parameters, lowPassCutoffId,
                                              kDefaultBypassLowPassCutoff);
  normalized[3] = getNormalizedParameterValue(parameters, lowPassSlopeId,
                                              kDefaultBypassLowPassSlope);
...
float getDistortionAmount(juce::AudioProcessorValueTreeState &parameters) {
  return getNormalizedParameterValue(
      parameters, adaptive_echo::plugin_parameters::kDistortionAmountId,
      kDefaultBypassDistortion);
}
} // namespace
...
AdaptiveEchoAudioProcessor::createParameterLayout() {
  ...
  params.push_back(std::make_unique<juce::AudioParameterFloat>(
      ...
      "Reference Frequency", frequencyRange, kDefaultReferenceFrequencyHz));
  ...
  params.push_back(std::make_unique<juce::AudioParameterBool>(
      adaptive_echo::plugin_parameters::kSingleVoiceId, "Single Voice",
      kDefaultSingleVoice));
  params.push_back(std::make_unique<juce::AudioParameterBool>(
      adaptive_echo::plugin_parameters::kConstantVelocityId,
      "Constant Velocity", kDefaultConstantVelocity));
  params.push_back(std::make_unique<juce::AudioParameterFloat>(
      adaptive_echo::plugin_parameters::kPreHighPassCutoffId,
      "Pre High-Pass Cutoff", normalizedRange, kDefaultBypassHighPassCutoff));
  params.push_back(std::make_unique<juce::AudioParameterFloat>(
      adaptive_echo::plugin_parameters::kPreHighPassSlopeId,
      "Pre High-Pass Slope", normalizedRange, kDefaultBypassHighPassSlope));
  params.push_back(std::make_unique<juce::AudioParameterFloat>(
      adaptive_echo::plugin_parameters::kPreLowPassCutoffId,
      "Pre Low-Pass Cutoff", normalizedRange, kDefaultBypassLowPassCutoff));
  params.push_back(std::make_unique<juce::AudioParameterFloat>(
      adaptive_echo::plugin_parameters::kPreLowPassSlopeId,
      "Pre Low-Pass Slope", normalizedRange, kDefaultBypassLowPassSlope));
  params.push_back(std::make_unique<juce::AudioParameterFloat>(
      adaptive_echo::plugin_parameters::kHighPassCutoffId, "High-Pass Cutoff",
      normalizedRange, kDefaultBypassHighPassCutoff));
  params.push_back(std::make_unique<juce::AudioParameterFloat>(
      adaptive_echo::plugin_parameters::kHighPassSlopeId, "High-Pass Slope",
      normalizedRange, kDefaultBypassHighPassSlope));
  params.push_back(std::make_unique<juce::AudioParameterFloat>(
      adaptive_echo::plugin_parameters::kLowPassCutoffId, "Low-Pass Cutoff",
      normalizedRange, kDefaultBypassLowPassCutoff));
  params.push_back(std::make_unique<juce::AudioParameterFloat>(
      adaptive_echo::plugin_parameters::kLowPassSlopeId, "Low-Pass Slope",
      normalizedRange, kDefaultBypassLowPassSlope));
  params.push_back(std::make_unique<juce::AudioParameterFloat>(
      adaptive_echo::plugin_parameters::kDistortionAmountId, "Distortion",
      normalizedRange, kDefaultBypassDistortion));
...
bool AdaptiveEchoAudioProcessor::decodeAudioFile(
    const juce::String &path, std::vector<float> &monoSamples,
  ...
  if (!reader->read(&buffer, 0, static_cast<int>(reader->lengthInSamples), 0,
                    true, true)) {
...
bool AdaptiveEchoAudioProcessor::savePresetToPath(const juce::String &path) {
  const auto presetFile = normalizePresetSaveFile(path);
  const auto parentDirectory = presetFile.getParentDirectory();
  auto previousPresetPath = juce::String();
  {
    const std::lock_guard<std::mutex> lock(stateMutex);
    previousPresetPath = currentPresetPath;
  }

  if ((!parentDirectory.exists() && !parentDirectory.createDirectory()) ||
      !parentDirectory.isDirectory()) {
    setStatusText("Unable to create the preset directory.");
    sendChangeMessage();
    return false;
  }
  ...
  {
    const std::lock_guard<std::mutex> lock(stateMutex);
    currentPresetPath = presetFile.getFullPathName();
  }

  const auto state = createStateTree();
  auto xml = state.createXml();
  if (xml == nullptr ||
      !presetFile.replaceWithText(xml->toString(), false, false, "\n")) {
    {
      const std::lock_guard<std::mutex> lock(stateMutex);
      currentPresetPath = previousPresetPath;
      statusText = "Unable to save the preset file.";
    }
    sendChangeMessage();
    return false;
  }

  {
    const std::lock_guard<std::mutex> lock(stateMutex);
    currentPresetPath = presetFile.getFullPathName();
    statusText = "Preset saved to " + presetFile.getFileName() + ".";
  }
...
bool AdaptiveEchoAudioProcessor::loadPresetFromPath(const juce::String &path) {
  const auto presetFile = resolvePresetLoadFile(path);
  if (!presetFile.existsAsFile()) {
    setStatusText("Preset file not found.");
    sendChangeMessage();
    return false;
  }
  ...
  if (xml == nullptr) {
    setStatusText("Unable to read the preset file.");
    sendChangeMessage();
    return false;
  }
  ...
  if (!stateTree.isValid()) {
    setStatusText("Preset data is invalid.");
    sendChangeMessage();
    return false;
  }

  stateTree.setProperty(kPresetPathProperty, presetFile.getFullPathName(),
                        nullptr);
  restoreStateFromTree(stateTree);
  updateHostDisplay();
  return true;
}
...
void AdaptiveEchoAudioProcessor::beginTraining() {
  ...
  trainingThread = std::thread([this, sample = std::move(sampleSnapshot),
    ...
    auto result = adaptive_echo::train_synth(
        sample, trainingPopulationSize, trainingInitialSigma,
        selectedTrainingTimeLimitSeconds, false,
    ...
    const auto bestSettings = result.best_settings;
    {
      ...
      statusText = juce::String::formatted(
          "Training complete. Best loss %.4f after %d generations.",
...
void AdaptiveEchoAudioProcessor::syncEffectParametersFromSettings(
  ...
  setNormalizedParameterValue(
      ...
      kDefaultBypassHighPassCutoff);
  setNormalizedParameterValue(
      parameters, adaptive_echo::plugin_parameters::kPreHighPassSlopeId,
      kDefaultBypassHighPassSlope);
  setNormalizedParameterValue(
      parameters, adaptive_echo::plugin_parameters::kPreLowPassCutoffId,
      kDefaultBypassLowPassCutoff);
  setNormalizedParameterValue(
      parameters, adaptive_echo::plugin_parameters::kPreLowPassSlopeId,
      kDefaultBypassLowPassSlope);
  setNormalizedParameterValue(
      parameters, adaptive_echo::plugin_parameters::kHighPassCutoffId,
      settings[adaptive_echo::constants::HIGH_PASS_CUTOFF_INDEX]);
  setNormalizedParameterValue(
      parameters, adaptive_echo::plugin_parameters::kHighPassSlopeId,
      settings[adaptive_echo::constants::HIGH_PASS_SLOPE_INDEX]);
  setNormalizedParameterValue(
      parameters, adaptive_echo::plugin_parameters::kLowPassCutoffId,
      settings[adaptive_echo::constants::LOW_PASS_CUTOFF_INDEX]);
  setNormalizedParameterValue(
      parameters, adaptive_echo::plugin_parameters::kLowPassSlopeId,
      settings[adaptive_echo::constants::LOW_PASS_SLOPE_INDEX]);
  setNormalizedParameterValue(
      parameters, adaptive_echo::plugin_parameters::kDistortionAmountId,
      settings[adaptive_echo::constants::DISTORTION_INDEX]);
...
juce::String AdaptiveEchoAudioProcessor::getPresetPath() const {
  const std::lock_guard<std::mutex> lock(stateMutex);
...
juce::String AdaptiveEchoAudioProcessor::getTrainingProgressText() const {
  ...
  const auto trainingTimeLimitSeconds =
      activeTrainingTimeLimitSeconds.load() > 0.0f
          ? activeTrainingTimeLimitSeconds.load()
  ...
  if (trainingNow) {
    return juce::String::formatted("Generation %d  |  Loss %.4f  |  Sigma %.3f "
                                   " |  Evals %d  |  %.1fs / %.0fs",
                                   generation, bestLoss, sigma, evalCount,
                                   elapsedSeconds, trainingTimeLimitSeconds);
  }

  return juce::String::formatted("Completed in %.1fs  |  Generations %d  |  "
                                 "Loss %.4f  |  Sigma %.3f  |  Evals %d",
                                 elapsedSeconds, generation, bestLoss, sigma,
                                 evalCount);
}
...
AdaptiveEchoAudioProcessor::getParameters() {
  return parameters;
}

juce::File AdaptiveEchoAudioProcessor::getDefaultPresetDirectory() const {
  return juce::File::getSpecialLocation(juce::File::userDocumentsDirectory)
      .getChildFile(kDefaultPresetDirectoryName);
}
...
std::vector<float>
AdaptiveEchoAudioProcessor::getCurrentSettingsSnapshot() const {
  const std::lock_guard<std::mutex> lock(stateMutex);
...
bool AdaptiveEchoAudioProcessor::isSingleVoiceEnabled() const {
  return getBoolParameterValue(parameters,
                               adaptive_echo::plugin_parameters::kSingleVoiceId,
                               kDefaultSingleVoice);
}

bool AdaptiveEchoAudioProcessor::isConstantVelocityEnabled() const {
  return getBoolParameterValue(
      parameters, adaptive_echo::plugin_parameters::kConstantVelocityId,
...
void AdaptiveEchoAudioProcessor::startVoiceRelease(ActiveVoice &voice) {
  const auto fadeSamples = std::max(
      1.0, currentSampleRate * static_cast<double>(kSingleVoiceFadeSeconds));
...
void AdaptiveEchoAudioProcessor::startVoice(int midiNote, float velocity) {
  ...
  const bool pitchTrackOscA = getBoolParameterValue(
      parameters, adaptive_echo::plugin_parameters::kOscAPitchTrackId,
      kDefaultOscAPitchTrack);
  const bool pitchTrackOscB = getBoolParameterValue(
      parameters, adaptive_echo::plugin_parameters::kOscBPitchTrackId,
      kDefaultOscBPitchTrack);

  auto rendered = adaptive_echo::render_note_audio(
      settings, getReferenceFrequency(), midiNote, currentSampleRate,
      pitchTrackOscA, pitchTrackOscB);
  ...
  if (activeVoices.size() >= static_cast<size_t>(maxPolyphony)) {
    auto oldest =
        std::min_element(activeVoices.begin(), activeVoices.end(),
...
void AdaptiveEchoAudioProcessor::mixActiveVoices(
  ...
  for (auto voice = activeVoices.begin(); voice != activeVoices.end();) {
    ...
    for (size_t i = 0; i < blockSamples; ++i) {
      const auto sample =
          voice->samples[voice->position + i] * voice->velocity * voice->gain;
      left[i] += sample;
      if (right != nullptr) {
        right[i] += sample;
      }
...
juce::ValueTree AdaptiveEchoAudioProcessor::createStateTree() {
  ...
  state.setProperty(kPresetPathProperty, currentPresetPath, nullptr);
  state.setProperty(
      kTrainedSettingsProperty,
      juce::String(adaptive_echo::serialize_settings(trainedSettings)),
      nullptr);
  return state;
}

void AdaptiveEchoAudioProcessor::restoreStateFromTree(
  ...
  const auto restoredPath =
      stateTree.getProperty(kSamplePathProperty).toString();
  const auto restoredPresetPath =
      stateTree.getProperty(kPresetPathProperty).toString();
  ...
  {
    const std::lock_guard<std::mutex> lock(stateMutex);
    currentPresetPath = restoredPresetPath;
    trainedSettings = restoredSettings;
    loadedSample.clear();
    samplePath.clear();
    if (restoredPath.isEmpty()) {
      statusText = "Preset restored.";
    }
  }
  ...
  if (restoredPath.isNotEmpty()) {
    if (!loadSampleFromPath(restoredPath)) {
      const std::lock_guard<std::mutex> lock(stateMutex);
      samplePath = restoredPath;
      statusText = "Saved sample path could not be reloaded. Trained settings "
                   "were restored.";
    }
  }
